...
(function() {
  ...
  if (typeof window !== 'undefined' && window.__FFM_CONTENT_MAIN_ALREADY_LOADED__) {
    try { console.log("[AutoList Pro content] Duplicate content_main.js load blocked."); } catch (e) {}
  } else {
...
function ffmDataUrlToBlob(dataUrl, type) {
  try {
    ...
    try { console.warn('[AutoList Pro content] ffmDataUrlToBlob failed', e); } catch (err) {}
    return null;
  }
}
...
(function ffmContentProbe() {
  try {
    ...
    console.log('[AutoList Pro content] content_main.js loaded', {
      href,
      hasFileInput
    });
  } catch (e) {
    console.log('[AutoList Pro content] content_main.js loaded (no DOM access)', String(e));
  }
})();
...
try {
    ...
    try {
      chrome.storage.local.get(['ffm_auto_publish_enabled','auto_publish_enabled'], (res) => {
        try {
          ...
          console.log('[AutoList Pro content] persistent auto-publish loaded =', window.ffm_allow_auto_publish);
        } catch (e) {}
      });
    } catch (e) {}
  }
  ...
  // Defensive cleanup: remove stray debug/banner elements that may be left
  // behind by older builds or debug flags (prevents a visible "content script"
  // banner during CLFP/DnR flows). Keep minimal and resilient.
  (function ffmRemoveDebugBanners() {
    try {
      const ids = ['extension-pin-panel','ffm-pin-iframe','ffm-debug-overlay','ffm-hide-confirm-overlay','__ffm_active_listings_banner','ffm-overlay','ffm-as-overlay'];
      ids.forEach(id => { try { const el = document.getElementById(id); if (el && el.parentNode) el.parentNode.removeChild(el); } catch (e) {} });

      // Remove any element whose visible text explicitly advertises a content-script load
      try {
        ...
        if (root) {
          ...
          while (node) {
            try {
              const txt = (node.textContent || '').toString().trim();
              if (txt && /content script loaded|fast4mp content script loaded/i.test(txt)) removeList.push(node);
            } catch (e) {}
...
window.addEventListener('ffmAttachNow', async () => {
  try {
    ...
    try {
      if (window.__ffm_attach_in_progress) {
        console.debug && console.debug('[AutoList Pro content] ffmAttachNow: attach already in progress, skipping');
        return;
      }
    ...
    try {
      ...
      console.log('[AutoList Pro content] attach forwarded to correct frame');
      ...
      try {
        ...
        if (!input) {
          console.warn('[AutoList Pro content] ffmAttachNow: no file input found in this frame');
        } else {
          try {
            await ffmAttachMediaToFileInput(input, files);
          } catch (e) {
            console.warn('[AutoList Pro content] ffmAttachNow attach failed', e);
          }
        }
      } catch (e) {
        console.warn('[AutoList Pro content] ffmAttachNow attach failed', e);
  ...
  } catch (e) { console.warn('[AutoList Pro content] ffmAttachNow handler error', e); }
});
...
try { console.debug && console.debug('[AutoList Pro content] content_main loaded, registering listeners'); } catch (e) {}
...
try {
  if (chrome && chrome.runtime && typeof chrome.runtime.sendMessage === 'function') {
    const sendReady = (isTop) => {
      try { chrome.runtime.sendMessage({ action: 'ffm_content_ready', isTop: !!isTop }, () => {}); } catch (e) {}
    };

    // Top frame: announce readiness immediately and retry a few times
    // to increase odds the service worker receives the signal early.
    try {
      const isTop = (window && window.self === window.top);
      if (isTop) {
        // Immediate + scheduled retries
        sendReady(true);
        setTimeout(() => sendReady(true), 300);
        setTimeout(() => sendReady(true), 900);
        // Also announce again after full load event in case scripts run earlier
        try { window.addEventListener && window.addEventListener('load', () => { try { sendReady(true); } catch (e) {} }); } catch (e) {}
        // Connect a long-lived port so background can push messages reliably
        try {
          if (chrome && chrome.runtime && typeof chrome.runtime.connect === 'function' && !window.__ffm_content_port_connected) {
            try {
              const __ffm_content_port = chrome.runtime.connect({ name: 'ffm_content_port' });
              window.__ffm_content_port = __ffm_content_port;
              window.__ffm_content_port_connected = true;
              __ffm_content_port.onMessage.addListener((msg) => {
                try {
                  if (msg && msg.action === 'ffm_from_published_media_result') {
                    try { window.__ffm_published_media_latest = msg.data || null; } catch (e) {}
                    try { window.dispatchEvent(new CustomEvent('ffm_published_media', { detail: window.__ffm_published_media_latest })); } catch (e) {}
                    // Acknowledge receipt so background can clear queued media for this tab
                    try { chrome.runtime && chrome.runtime.sendMessage && chrome.runtime.sendMessage({ action: 'ffm_from_published_media_ack' }, () => {}); } catch (e) {}
                  }
                } catch (e) {}
              });
              __ffm_content_port.onDisconnect.addListener(() => { try { window.__ffm_content_port_connected = false; delete window.__ffm_content_port; } catch (e) {} });
            } catch (e) {}
          }
        } catch (e) {}
      } else {
        // Non-top frames send a single lightweight ping
        sendReady(false);
      }
    } catch (e) {
      try { sendReady(false); } catch (er) {}
    }
  }
} catch (e) {}
...
(async function ffmCheckForStagedListing() {
  try {
    ...
    if (!incoming) return;
    try { console.log('[AutoList Pro] Detected staged listing', incoming); } catch (e) {}
    // ------------------------------------------------------
    ...
    try { if (typeof ffmRunPublishFlow === 'function') ffmRunPublishFlow(incoming); } catch (e) { console.debug('[AutoList Pro] ffmRunPublishFlow missing or errored', e); }
  } catch (e) { console.debug('[AutoList Pro] ffmCheckForStagedListing error', e); }
})();
    ...
    chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
      try {
        if (msg && msg.action === 'publish-listing') {
          try { console.log('[AutoList Pro] publish-listing received', msg); } catch (e) {}
          try {
            if (typeof ffmHandlePublishRequest === 'function') {
              ffmHandlePublishRequest(msg);
            } else {
              console.warn('[AutoList Pro] ffmHandlePublishRequest not found');
            }
          } catch (e) { console.error('[AutoList Pro] ffmHandlePublishRequest threw', e); }
          try { sendResponse && sendResponse({ ok: true }); } catch (e) {}
          return true;
        }
      } catch (e) {}
  ...
  if (chrome && chrome.runtime && chrome.runtime.onMessage) {
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      try {
        ...
        try { console.debug && console.debug('[AutoList Pro content] perform-generate-scrape (top-level) received from', sender && sender.tab ? sender.tab.id : sender); } catch (e) {}
        (async () => {
          try {
            ...
            try { console.debug && console.debug('[AutoList Pro content] perform-generate-scrape (top-level) result', result); } catch (e) {}
            try { sendResponse && sendResponse(result); } catch (e) { try { console.debug && console.debug('[AutoList Pro content] perform-generate-scrape (top-level) sendResponse failed', e); } catch (er) {} }
          } catch (e) { try { console.debug && console.debug('[AutoList Pro content] perform-generate-scrape (top-level) error', e); } catch (er) {} try { sendResponse && sendResponse(null); } catch (er) {} }
        })();
        return true;
      } catch (e) {}
    });
    ...
    chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
      try {
        ...
        try {
            ...
            if (msg.hasStagedMedia) {
              try {
                chrome.storage.local.get(['ffm_staged_relist_media'], async (res) => {
                  ...
                  try {
                    console.log('[AutoList Pro content] Injecting staged relist payload (from message + staged media)', data.title || data.inventoryName);
                    ...
                    try {
                      ...
                      try {
                        ...
                        if (imgs && imgs.length) {
                            ...
                            try {
                              ...
                              if (typeof fn === 'function') {
                                for (const img of imgs) {
                                  try { await fn(img); } catch (e) { console.warn('[AutoList Pro content] image upload helper failed', fnName, e); }
                                }
                                used = true;
                                break;
                              }
                          ...
                          if (!used) {
                            console.log('[AutoList Pro content] relist payload contains images but no upload helper found; images are preserved in storage under ffm_relist_payload', imgs.length);
                          }
                        }
                      } catch (e) { console.warn('[AutoList Pro content] image injection error', e); }

                      try { sendResponse && sendResponse({ ok: true }); } catch (e) {}
                    } catch (e) { console.warn('[AutoList Pro content] Failed to inject relist payload', e); try { sendResponse && sendResponse({ ok: false, error: String(e) }); } catch (er) {} }
                ...
                console.warn('[AutoList Pro content] failed to read staged media', e);
                // fall through to treat message.payload as-is
              }
            }
            // No staged media, but have a payload — apply directly
            try {
              console.log('[AutoList Pro content] Injecting relist payload (from message)', data.title || data.inventoryName);
              try { chrome.storage.local.set({ ffm_relist_payload: data }); } catch (e) {}
              const titleEl = document.querySelector('input[name="title"], input[aria-label*="title" i], input[placeholder*="title" i]');
              if (titleEl) try {
                titleEl.value = (data.title || data.inventoryName || '').trim();
          ...
          try {
            chrome.storage.local.get(['ffm_relist_payload', 'ffm_staged_listing'], async (res) => {
              try {
                      ...
                      try {
                        chrome.storage.local.get(['ffm_publish_' + publishId], async (pubRes) => {
                          try {
                            ...
                            if (!full) {
                              try { console.warn('[AutoList Pro content] no relist/staged payload found in storage'); } catch (e) {}
                              try { sendResponse && sendResponse({ ok: false, reason: 'no-payload' }); } catch (e) {}
                              return;
                            }
                            data = full;
                            try { console.log('[AutoList Pro content] Injecting saved listing data for relist (from ffm_publish)', data.title || data.inventoryName); } catch (e) {}
                            ...
                            try {
                              ...
                              try {
                                ...
                                if (imgs && imgs.length) {
                                  try { chrome.storage.local.set({ ffm_relist_payload: data }); } catch (e) {}
                                  try { console.log('[AutoList Pro content] relist payload contains images; preserved under ffm_relist_payload', imgs.length); } catch (e) {}
                                }
                              } catch (e) {}
                            } catch (e) {}
                            try { sendResponse && sendResponse({ ok: true }); } catch (e) {}
                ...
                console.log('[AutoList Pro content] Injecting saved listing data for relist', data.title || data.inventoryName);
                try {
                  // Title
                  const titleEl = document.querySelector('input[name="title"], input[aria-label*="title" i], input[placeholder*="title" i]');
                  if (titleEl) try { titleEl.value = data.title || data.inventoryName || ''; } catch (e) {}
          ...
          } catch (e) { console.warn('[AutoList Pro content] ffm_inject_relist_payload handler failed', e); }
          return true; // keep channel open while storage read completes
        } catch (e) {}
      } catch (e) {}
    });
  }
} catch (e) { console.debug('[AutoList Pro content] install ffm_inject_relist_payload listener failed', e); }
      ...
      try {
        if (chrome && chrome.runtime && chrome.runtime.onMessage) {
          chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
            try {
              ...
              if (msg.action === "ffm-start-select-images") {
                try { console.debug && console.debug('[AutoList Pro content] Received ffm-start-select-images → activating picker'); } catch (e) {}
                try { startSharetownSelectMode(); } catch (e) { console.error('[AutoList Pro content] startSharetownSelectMode failed', e); }
                try { sendResponse && sendResponse({ ok: true }); } catch (e) {}
                return true;
              }
            } catch (e) {}
          ...
          } catch (e) { console.debug('[AutoList Pro content] install ffm_dnr_try_title_search listener failed', e); }
    ...
    return { success: false, fallback: true, reason: 'exception', error: String(e) };
          ...
          async function ffmRunDeleteSequenceOnCard(cardNode) {
            try {
              ...
              // Wait for confirmation modal to exist in the DOM
              try { console.debug && console.debug('[DnR] waiting for delete-listing modal, visibility=', document.visibilityState, 'hasFocus=', document.hasFocus && document.hasFocus()); } catch(e){}
              let ffmRequestedFocus = false;
              for (let i = 0; i < 25; i++) {
                try {
                  const modalHeader = Array.from(document.querySelectorAll('span, h2, div'))
                    .find(el => (el.textContent || '').trim().toLowerCase() === 'delete listing');
                  if (modalHeader) {
                    try { console.debug && console.debug('[DnR] modalHeader found; visible=', modalHeader.offsetParent !== null); } catch(e){}
                    // If the tab/window is not visible, request the background to bring it forward once
                    try {
                      if (!ffmRequestedFocus && (document.visibilityState !== 'visible' || (document.hasFocus && !document.hasFocus()))) {
                        ffmRequestedFocus = true;
                        try { chrome.runtime.sendMessage({ action: 'ffm_bring_tab_foreground' }, () => {}); } catch(e){}
                        try { console.debug && console.debug('[DnR] requested foreground focus from background'); } catch(e){}
                        // give the browser a short moment to focus before proceeding
                        await sleep(300);
                      }
                    } catch(e){}
                    break;
              ...
              // Click confirm button, with keyboard fallbacks if clicks are ignored while backgrounded
              let confirmBtn = Array.from(document.querySelectorAll("div[role='button'], button, span"))
                .find(el => {
                  try {
                    const aria = (el.getAttribute && el.getAttribute('aria-label'))?.toLowerCase() || '';
                    const text = (el.innerText || '').trim().toLowerCase();
                    return (aria === 'delete' || text === 'delete' || text.includes('delete'));
                  } catch (e) { return false; }
                });

              const dispatchKey = (target, key) => {
                try {
                  const kd = new KeyboardEvent('keydown', { key: key, code: key, keyCode: key === 'Enter' ? 13 : (key === 'Tab' ? 9 : 0), which: key === 'Enter' ? 13 : (key === 'Tab' ? 9 : 0), bubbles: true, cancelable: true });
                  const ku = new KeyboardEvent('keyup', { key: key, code: key, keyCode: key === 'Enter' ? 13 : (key === 'Tab' ? 9 : 0), which: key === 'Enter' ? 13 : (key === 'Tab' ? 9 : 0), bubbles: true, cancelable: true });
              ...
              if (confirmBtn) {
                try {
                  console.debug && console.debug('[DnR] confirmBtn located — attempting mouse events then keyboard fallback, visibility=', document.visibilityState, 'hasFocus=', document.hasFocus && document.hasFocus());
                  try { confirmBtn.scrollIntoView && confirmBtn.scrollIntoView({ behavior: 'auto', block: 'center' }); } catch(e){}
                  ...
                  try { confirmBtn.click && confirmBtn.click(); } catch(e){}

                  // Wait briefly then check; if still present try keyboard Enter on the button and as fallback Tab+Enter
                  await sleep(200);
                  let stillThere = Array.from(document.querySelectorAll('span, h2')).some(el => (el.textContent || '').trim().toLowerCase() === 'delete listing');
                  if (stillThere) {
                    try { console.debug && console.debug('[DnR] click did not close modal; attempting focus + Enter on button'); } catch(e){}
                    try { confirmBtn.focus && confirmBtn.focus(); } catch(e){}
                    try { dispatchKey(confirmBtn, 'Enter'); } catch(e){}
                    await sleep(200);
                    stillThere = Array.from(document.querySelectorAll('span, h2')).some(el => (el.textContent || '').trim().toLowerCase() === 'delete listing');
                    if (stillThere) {
                      try { console.debug && console.debug('[DnR] Enter on button failed; attempting Tab+Enter sequence'); } catch(e){}
                      try { document.body.focus && document.body.focus(); } catch(e){}
                      try { dispatchKey(document.body, 'Tab'); } catch(e){}
                      await sleep(150);
                      try { dispatchKey(document.body, 'Enter'); } catch(e){}
                      await sleep(300);
                      stillThere = Array.from(document.querySelectorAll('span, h2')).some(el => (el.textContent || '').trim().toLowerCase() === 'delete listing');
                    }
                  }

                  // If modal still present, allow further retry loop below
                } catch (e) { console.debug && console.debug('[DnR] confirmBtn click/keyboard attempts error', e); }
              } else {
                const fallback = document.querySelector("div[aria-label='Delete'][role='button']");
                if (fallback) {
                  try {
                    console.debug && console.debug('[DnR] fallback delete button located; attempting click then Enter fallback');
                    try { fallback.scrollIntoView && fallback.scrollIntoView({ behavior: 'auto', block: 'center' }); } catch(e){}
                    try { ['mouseover','mousedown','mouseup','click'].forEach(t => fallback.dispatchEvent(new MouseEvent(t, { bubbles: true, cancelable: true, view: window }))); } catch(e){}
                    try { fallback.click && fallback.click(); } catch(e){}
                    await sleep(200);
                    let stillThere = Array.from(document.querySelectorAll('span, h2')).some(el => (el.textContent || '').trim().toLowerCase() === 'delete listing');
                    if (stillThere) {
                      try { fallback.focus && fallback.focus(); } catch(e){}
                      try { dispatchKey(fallback, 'Enter'); } catch(e){}
                      await sleep(200);
                      stillThere = Array.from(document.querySelectorAll('span, h2')).some(el => (el.textContent || '').trim().toLowerCase() === 'delete listing');
                      if (stillThere) {
                        try { dispatchKey(document.body, 'Tab'); } catch(e){}
                  ...
                  } catch(e) { console.debug && console.debug('[DnR] fallback attempts error', e); }
                }
              }
            ...
            try {
              ...
              const handler = (e) => {
                try {
                  ...
                  try { console.debug && console.debug('[AutoList Pro content] Item selected:', name); } catch (er) {}
                  try { chrome.runtime.sendMessage({ action: 'sharetown_item_selected', data: { name } }); } catch (er) {}
                } catch (err) { console.error('[AutoList Pro content] select handler failed', err); }
              };
              el.addEventListener('click', handler, { once: true });
            } catch (e) {}
          });
        } catch (err) {
          console.error('[AutoList Pro content] startSharetownSelectMode failed', err);
        }
      }
...
// ===== AutoList Pro: Auto Active Listings refresh (robust + debug) =====
(function() {
    ...
    chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
      try {
        ...
        if (msg.action === 'ffmActiveListingsAuto') {
          try { console.log('[AutoList Pro content] Received ffmActiveListingsAuto'); } catch (e) {}
        ...
        if (msg.action === 'ffm_as_prepare_hidden') {
          FFM_AS_HIDDEN = true;
          try { console.log('[AutoList Pro AS] Hidden mode enabled'); } catch (e) {}
          try { sendResponse && sendResponse({ ok: true }); } catch (e) {}
        ...
        if (msg.action === 'ffm_run_active_sync_hidden') {
          (async () => {
            ...
            try { console.log('[AutoList Pro AS] Starting hidden scrape (handshake)'); } catch (e) {}
            ...
            try {
              ...
              if (scroller) {
                ...
                try { await (typeof window.ffmAsScrollSellingUntilStable === 'function' ? window.ffmAsScrollSellingUntilStable() : ffmAsScrollSellingUntilStable()); } catch (e) { try { console.warn('[AutoList Pro AS] ffmAsScrollSellingUntilStable failed', e); } catch (er) {} }
              }
            } catch (e) {}
            ...
            try {
              ...
              try { console.warn('[AutoList Pro AS] ffmScrapeActiveListingsOnce error (hidden run)', e); } catch (er) {}
              try { await ffmAsDebugHold(15000); } catch (e) {}
      ...
      } catch (e) { try { console.warn('[AutoList Pro AS] onMessage error', e); } catch (er) {} }
    });
  } catch (e) {}
  ...
  (async function ffmAutorunHiddenASFromUrl(){
    try {
      ...
      try { console.log('[AutoList Pro AS] URL-driven hidden AS detected, reqId=', reqId); } catch (e) {}
      ...
      try {
        ...
        try {
          ...
          if (isHiddenAS) {
            ...
            try { await ffmAsWaitUntilAllActiveListingsPresent(); } catch (e) { try { console.warn('[AutoList Pro AS] ffmAsWaitUntilAllActiveListingsPresent failed', e); } catch (er) {} }
          }
        } catch (e) {}
        ...
        try {
          ...
          try { console.warn('[AutoList Pro AS] scraper invocation failed', e); } catch (er) {}
          results = null;
        }
  ...
  chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
    if (msg && msg.action === 'ffmActiveListingsAuto') {
      try { console.log('[AutoList Pro content] Received ffmActiveListingsAuto — running auto-refresh'); } catch (e) {}
        ...
        if (typeof window.ffmScrapeActiveListingsOnce === 'function') {
          try {
            window.ffmScrapeActiveListingsOnce().then(() => {
              ...
              try { console.warn('[AutoList Pro content] ffmScrapeActiveListingsOnce threw', err); } catch (e) {}
              try { sendResponse && sendResponse({ ok: false, error: String(err) }); } catch (e) {}
            });
            return true; // keep port open for async response
          } catch (e) {
            try { console.warn('[AutoList Pro content] error invoking ffmScrapeActiveListingsOnce', e); } catch (er) {}
          }
        }
  ...
  async function ffmAutoActiveListingsRefresh(forced = false) {
    ...
    console.log(`[AutoList Pro content] Performing auto Active Listings refresh…${forced ? " (forced)" : ""}`);
    ...
    try {
      ...
      try {
        if (window.__ffm_as_finalized) {
          try { console.log('[AutoList Pro content] Auto-refresh ignored — already finalized'); } catch (e) {}
      ...
      for (let i = 0; i < maxScrolls; i++) {
        ...
        console.log(`[AutoList Pro content][auto] probe ${i+1}/${maxScrolls}: candidateCards=${count} (best=${bestSeen})`);
      ...
      while (Date.now() - start < maxRetryMs) {
        ...
        console.log(`[AutoList Pro content][auto] attempt ${attempts}: scraped=${scraped.length}`);
      ...
      console.log(`[AutoList Pro content] Active Listings refreshed — ${scraped.length} (attempts=${attempts}, ${elapsed}ms)`);
      ...
      console.error('[AutoList Pro content] Auto refresh error:', err);
        ...
        chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
          try {
            if (msg && msg.action === 'exitSelectMode') {
              try { window.ffm_panel_disabled = false; console.debug && console.debug('[AutoList Pro content] Panel re-enabled after exitSelectMode'); } catch (e) {}
              try { sendResponse && sendResponse({ ok: true }); } catch (e) {}
              return true;
            }
          } catch (e) {}
...
function computeCssLocal(el) {
  ...
  try {
    ...
    console.warn('[AutoList Pro] computeCssLocal failed', err);
    return '';
  }
}
...
try { if (window && window.ffm_debug) console.log('AutoList Pro content script loaded on page'); } catch (_) {}
      ...
      try {
        ...
        try {
          const sellerInfoEl = Array.from(document.querySelectorAll('*')).find(el => {
            ...
            try { if ((typeof window !== 'undefined') && (window.FFM_DEBUG || window.ffm_debug)) console.debug('[AutoList Pro FB-Images] Seller information cutoff Y:', cutoff); } catch (e) {}
          }
        ...
        for (const img of imgs) {
          try {
            ...
            try {
              ...
              if (absoluteY >= cutoff) {
                try { if ((typeof window !== 'undefined') && (window.FFM_DEBUG || window.ffm_debug)) console.debug('[AutoList Pro FB-Images] Skipped image below Seller info cutoff', src); } catch (e) {}
                continue;
              }
            } catch (e) {}
            urls.add(src);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'populate-fb-trigger') {
    try {
      ...
      if (!listing) { console.warn('populate-fb-trigger received but no staged listing found'); return; }
        ...
        // ----- AutoList Pro: Option A (Scheduler Detected - NO AUTO-PUBLISH) -----
        try {
          if (message.scheduleTaskId || window.ffm_is_scheduled_publish === true) {
            try { console.log('[AutoList Pro content] Scheduler detected → not forcing auto-publish'); } catch (e) {}
        ...
        } catch (err) { console.debug('[AutoList Pro content] scheduler detection noop failed', err); }
        ...
        try {
          ...
          try {
            if ((message && message.scheduleTaskId) || (window && window.ffmCurrentPopulate && window.ffmCurrentPopulate.scheduleTaskId) || (window && window.ffm_staged_publish_request_id)) {
              try { console.log('[AutoList Pro] Scheduled Publish detected (id-only) — not forcing auto-publish'); } catch (e) {}
              try { if (!window.ffmCurrentPopulate) window.ffmCurrentPopulate = {}; } catch (e) {}
              try { window.ffmCurrentPopulate.scheduleTaskId = message.scheduleTaskId || window.ffmCurrentPopulate.scheduleTaskId || true; } catch (e) {}
            }
          } catch (e) { console.debug('[AutoList Pro] scheduled id-only detection noop failed', e); }
  ...
  try {
    ...
    if ((message && message.scheduleTaskId) || (window && window.ffmCurrentPopulate && window.ffmCurrentPopulate.scheduleTaskId) || (window && window.ffm_staged_publish_request_id)) {
      try { console.log("[AutoList Pro] Scheduled publish detected — forcing autoPublish = true (main branch)"); } catch (e) {}
  ...
  const setTitle = (text) => {
                ...
                if (pub && pub.offsetParent !== null) {
                  try {
                    if ((window.ffm_is_scheduled_publish === true) || (window.ffmForceSchedulerAutoPublish === true)) {
                      try { console.log('[AutoList Pro content] Scheduler override: clicking Publish button now'); } catch (e) {}
                    }
      ...
      // Generic waiter: wait for an input to appear and be visible
      async function waitForInput(findFn, { timeoutMs = 5000, pollMs = 100 } = {}) {
        const start = Date.now();
        while (Date.now() - start < timeoutMs) {
          try {
            const el = (typeof findFn === 'function') ? findFn() : null;
      ...
      // Generic predicate waiter: wait until predicate() -> true or timeout
      async function waitFor(predicate, { timeoutMs = 5000, pollMs = 100 } = {}) {
        const start = Date.now();
        while (Date.now() - start < timeoutMs) {
          try {
            if (typeof predicate === 'function' && predicate()) return true;
          } catch (e) {}
          await new Promise(r => setTimeout(r, pollMs));
        }
        return false;
      ...
      // Variant-safe input finders
      function findTitleInput() {
        try {
          return document.querySelector('input[aria-label*="Title" i]') ||
                 document.querySelector('textarea[aria-label*="Title" i]') ||
      ...
      function findPriceInput() {
        try {
          return document.querySelector('input[aria-label*="Price" i]') ||
      ...
      // -------------------------------
      // Marketplace input resolution
      // -------------------------------

      function findMarketplaceLabelSpan(labelText) {
        try {
          return Array.from(document.querySelectorAll('span')).find(s => (s.textContent || '').toString().trim() === labelText && s.id);
        } catch (e) { return null; }
      }

      function findMarketplaceInputByLabel(labelText) {
        try {
          const label = findMarketplaceLabelSpan(labelText);
          // 1) aria-labelledby
          if (label && label.id) {
            const byAria = document.querySelector(`[aria-labelledby="${label.id}"]`);
            if (byAria) return byAria;
          }
          // 2) nearby input fallback
          if (label) {
            const container = label.closest('div') || label.parentElement || label.closest('[role="group"]');
            if (container) {
              const nearby = container.querySelector('input[type="text"], textarea, [contenteditable="true"]');
              if (nearby) return nearby;
            }
          }
          // 3) last-resort visible text input
          return Array.from(document.querySelectorAll('input[type="text"]')).find(i => i.offsetParent !== null && i.getBoundingClientRect().width > 120) || null;
        } catch (e) { return null; }
      }

      async function waitForMarketplaceInput(findFn, { timeoutMs = 15000, pollMs = 100 } = {}) {
        const start = Date.now();
        while (Date.now() - start < timeoutMs) {
          try {
            const el = (typeof findFn === 'function') ? findFn() : null;
      ...
      // Reusable helper: set and confirm input value with retries. Accepts either a
      // direct `element` or a `findInput` function that returns the element.
      async function setAndConfirmInput({ label, findInput, element, value, normalize = v => v, maxAttempts = 6, delayMs = 250 }) {
        const target = normalize(value);
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
          let input = null;
          try { input = element || ((typeof findInput === 'function') ? findInput() : null); } catch (e) { input = null; }
          if (!input) {
            try { console.warn(`[AutoList Pro] ${label}: input not found (attempt ${attempt})`); } catch (e) {}
          } else {
            try {
              input.focus && input.focus();
            } catch (e) {}
            try {
              // For some FB controls, setting value directly is necessary
              input.value = value;
            } catch (e) {}
            try { input.dispatchEvent && input.dispatchEvent(new Event('input', { bubbles: true })); } catch (e) {}
            try { input.dispatchEvent && input.dispatchEvent(new Event('change', { bubbles: true })); } catch (e) {}

            await new Promise(r => setTimeout(r, delayMs));

            let actual = '';
            try { actual = normalize(input.value); } catch (e) { actual = '' + (input.value || ''); }
            if (actual === target) {
              try { console.log(`[AutoList Pro] ✅ ${label} confirmed`, actual); } catch (e) {}
              return true;
            }

            try { console.warn(`[AutoList Pro] ${label} mismatch (attempt ${attempt})`, { expected: target, actual }); } catch (e) {}
          }
          await new Promise(r => setTimeout(r, delayMs));
        }
        throw new Error(`[AutoList Pro] ❌ ${label} failed to confirm after ${maxAttempts} attempts`);
      }
      ...
      (async () => {
        ...
        try {
          ...
          console.log("[MEDIA_PORT] requesting media records for", listingId);
          ...
          async function ffmAttachStagedMedia(stagedFiles) {
            console.log('[AutoList Pro] ATTACH START — files:', stagedFiles.length);
            ...
            console.log('[AutoList Pro] Inputs:', {
              count: inputs.length,
            ...
            if (!imageInput && !videoInput) {
              console.warn('[AutoList Pro] ERROR — no uploader inputs found at all');
              return false;
            }
            ...
            if (imageFiles.length && imageInput) {
              ...
              console.log('[AutoList Pro] Attaching IMAGES:', imageFiles.length);
              try { imageInput.files = dtImg.files; } catch (e) { try { await ffmInjectFilesIntoFacebookInput(imageInput, imageFiles); } catch (ie) { console.warn('[AutoList Pro] injector errored', ie); } }
              try { imageInput.dispatchEvent(new Event('change', { bubbles: true })); } catch (e) {}
              console.log('[AutoList Pro] IMAGE attach complete');
            } else if (imageFiles.length && !imageInput) {
              console.warn('[AutoList Pro] WARNING — have images but no IMAGE uploader input');
            } else {
              console.log('[AutoList Pro] No images in stagedFiles.');
            }
            ...
            if (videoFiles.length && videoInput) {
              ...
              console.log('[AutoList Pro] Attaching VIDEO files:', videoFiles.length);
              try { videoInput.files = dtVid.files; } catch (e) { try { await ffmInjectFilesIntoFacebookInput(videoInput, videoFiles); } catch (ie) { console.warn('[AutoList Pro] injector errored', ie); } }
              try { videoInput.dispatchEvent(new Event('change', { bubbles: true })); } catch (e) {}
              hasVideo = true;
              console.log('[AutoList Pro] VIDEO attach complete');
            } else if (videoFiles.length && !videoInput) {
              console.warn('[AutoList Pro] WARNING — have videos but no VIDEO uploader input');
            } else {
              console.log('[AutoList Pro] No videos in stagedFiles.');
            }
          ...
          if (hasVideo) {
            console.log('[AutoList Pro] Video detected → waiting for preview');
            ...
            console.log('[AutoList Pro] No video → skipping preview wait');
          }
        ...
        async function ffmWaitForVideoUpload(uploaderInput, files) {
          console.log("[AutoList Pro] ffmWaitForVideoUpload start");
          ...
          } catch (e) { console.debug('[AutoList Pro] ffmWaitForVideoUpload: hasVideo infer error', e); }

          if (!hasVideo) {
            console.log("[AutoList Pro] ffmWaitForVideoUpload: no video files detected → skipping wait");
            return true;
          }
            ...
            try {
              ...
              if (preview) {
                try {
                  ...
                  if (src.indexOf('blob:') === 0 || src.indexOf('data:') === 0 || ready >= 3 || (dur > 0 && !isNaN(dur))) {
                    console.log("[AutoList Pro] ffmWaitForVideoUpload: preview ready (src/readyState/duration)", { srcPresent: !!src, readyState: ready, duration: dur });
                    return true;
                  }
                  // Otherwise, there is a preview element but it isn't ready yet — continue waiting
                  console.log('[AutoList Pro] ffmWaitForVideoUpload: preview present but not ready', { readyState: ready, duration: dur });
              ...
              try {
                ...
                if (vidInput && vidInput.files && vidInput.files.length > 0) {
                  console.log('[AutoList Pro] ffmWaitForVideoUpload: video input has file(s) attached', { count: vidInput.files.length });
                  ...
                  if (preview2) { console.log('[AutoList Pro] ffmWaitForVideoUpload: preview appeared after input attach'); return true; }
                }
              } catch (e) { /* ignore */ }
              ...
              try {
                ...
                if (prog) {
                  ...
                  // console.log('[AutoList Pro] ffmWaitForVideoUpload: upload in progress indicator present');
              ...
              console.debug('[AutoList Pro] ffmWaitForVideoUpload: iteration error', e);
            }

            await new Promise(r => setTimeout(r, poll));
          }

          console.warn("[AutoList Pro] ffmWaitForVideoUpload: TIMEOUT — continuing after", (Date.now() - start), "ms");
          return false;
        }
        ...
        async function ffmWaitForFBHydration(maxMs = 8000) {
          try {
            console.log('[AutoList Pro] ffmWaitForFBHydration: waiting for uploader input to appear');
            const start = Date.now();
            const interval = 300;
            ...
            console.warn('[AutoList Pro] ffmWaitForFBHydration: timed out after', maxMs, 'ms');
            return false;
          } catch (e) { console.warn('[AutoList Pro] ffmWaitForFBHydration error', e); return false; }
        }
        ...
        function ffmFindProperUploaderInput() {
          try {
            ...
            if (videoInput) {
              console.log("[AutoList Pro] Using VIDEO uploader input:", videoInput);
              return videoInput;
            }
            ...
            if (anyInput) {
              console.log("[AutoList Pro] Using fallback uploader input:", anyInput);
              return anyInput;
            }

            console.warn("[AutoList Pro] No suitable uploader input found (video or image).");
            return null;
          } catch (e) {
            console.error('[AutoList Pro] ffmFindProperUploaderInput error', e);
            return null;
          }
        }
...
function ffmFindRealUploaderInput() {
    ...
    console.warn("[AutoList Pro] REAL uploader input NOT found");
    return null;
}
  ...
  function ffmFindRealVideoInput() {
    try {
      ...
      console.log('[AutoList Pro] REAL video uploader:', real);
      return real || null;
    } catch (e) { console.debug('[AutoList Pro] ffmFindRealVideoInput error', e); return null; }
  }
...
async function ffmInjectSellingCardIdsFromSavedListings() {
  try {
    ...
    if (!titleToId.size) {
      console.log('[AutoList Pro] ffmInjectSellingCardIdsFromSavedListings: no listing map built');
      return;
    }
    ...
    console.log('[AutoList Pro] Injected ffmId on selling cards:', injectedCount);
  } catch (e) {
    try { console.warn('[AutoList Pro] ffmInjectSellingCardIdsFromSavedListings failed', e); } catch (_) {}
  }
}
  ...
  function ffmEnsurePhotoTabOpen() {
    try {
      ...
      if (addPhotosBtn && /Add photos/i.test(addPhotosBtn.textContent)) {
        ...
        if (btn) { btn.click(); console.log("[AutoList Pro] CLICKED Add Photos tab to activate uploader"); return true; }
      }
    } catch (e) { console.debug('[AutoList Pro] ffmEnsurePhotoTabOpen error', e); }
    console.warn("[AutoList Pro] Add Photos button not found");
    return false;
  }
  ...
  function ffmEnsureVideoTabOpen() {
    try {
      ...
      if (videoTab) {
        ...
        if (btn) { btn.click(); console.log("[AutoList Pro] CLICKED Add Video tab to activate uploader"); return true; }
      }
    } catch (e) { console.debug('[AutoList Pro] ffmEnsureVideoTabOpen error', e); }
    console.warn("[AutoList Pro] Add Video button not found");
    return false;
  }
...
function ffmForceReactClick(el) {
  ...
  } catch (e) { console.debug('[AutoList Pro] ffmForceReactClick error', e); }
}
...
function ffmFindHiddenVideoInput() {
  ...
  } catch (e) { console.debug('[AutoList Pro] ffmFindHiddenVideoInput error', e); return null; }
}
...
async function ffmAttachVideoToRealInput(files) {
  try {
    if (!files || !Array.isArray(files) || files.length === 0) {
      console.warn('[AutoList Pro] ffmAttachVideoToRealInput: no files provided');
      return false;
    }

    const input = ffmFindHiddenVideoInput();
    if (!input) {
      console.warn('[AutoList Pro] No REAL video input found.');
      return false;
    }

    console.log('[AutoList Pro] REAL video input found:', input);

    const dt = new DataTransfer();
    for (const f of files) dt.items.add(f);

    try { input.files = dt.files; } catch (e) { console.warn('[AutoList Pro] assignment to real input.files failed', e); }
    ...
    console.log('[AutoList Pro] Video assigned to REAL uploader:', input.files);
    return true;
  } catch (e) { console.debug('[AutoList Pro] ffmAttachVideoToRealInput error', e); return false; }
}
...
function ffmCreateInjectedVideoInput() {
  try {
    ...
    return inp;
  } catch (e) { console.debug('[AutoList Pro] ffmCreateInjectedVideoInput error', e); return null; }
}
...
async function ffmAttachVideoDirect(files) {
  try {
    if (!files || !Array.isArray(files) || files.length === 0) {
      console.warn('[AutoList Pro] ffmAttachVideoDirect: no files provided');
      return false;
    }

    // console.log('[AutoList Pro] Injecting our own video uploader…');

    const input = ffmCreateInjectedVideoInput();
    if (!input) {
      console.warn('[AutoList Pro] ffmAttachVideoDirect: failed to create injected input');
      return false;
    }
    ...
    try {
      const dt = new DataTransfer();
      for (const f of files) dt.items.add(f);
      try { input.files = dt.files; } catch (e) { console.warn('[AutoList Pro] direct assignment to injected input.files failed', e); }
    } catch (e) {
      console.warn('[AutoList Pro] ffmAttachVideoDirect: DataTransfer build failed', e);
    }

    console.log('[AutoList Pro] Injected uploader filelist:', input.files);

    // Now fire the change event that FB listens for
    try { input.dispatchEvent(new Event('change', { bubbles: true })); } catch (e) { try { input.dispatchEvent && input.dispatchEvent(new CustomEvent('change', { bubbles: true })); } catch (e2) {} }

    console.log('[AutoList Pro] Video attached via injected uploader.');
    return true;
  } catch (e) { console.debug('[AutoList Pro] ffmAttachVideoDirect error', e); return false; }
}
          ...
          function ffmChooseCorrectUploader(stagedFiles) {
            try {
              ...
              if (photos && !videos) {
                console.log('[AutoList Pro] Using IMAGE uploader (photos only)');
                return imageInput;
              }

              // 2. Only videos → use video input
              if (videos && !photos) {
                console.log('[AutoList Pro] Using VIDEO uploader (videos only)');
                return videoInput;
              }

              // 3. Both exist → use IMAGE first (Facebook often expects a photo)
              if (photos && videos) {
                console.log('[AutoList Pro] Using IMAGE uploader (both exist)');
                return imageInput;
              }

              console.warn('[AutoList Pro] No media found in stagedFiles');
              return imageInput || videoInput || null;
            } catch (e) { console.warn('[AutoList Pro] ffmChooseCorrectUploader error', e); return null; }
          }
        ...
        // Helper: request media from background via long-lived Port (avoids sendMessage quirks)
        function ffmRequestMediaViaPort({ listingId, userId }) {
          return new Promise((resolve) => {
            try {
              const port = chrome.runtime.connect({ name: "ffm_media_port" });
              ...
              port.postMessage({ action: "ffm_get_media_port", listingId, userId });
            } catch (e) {
        ...
        async function ffmGetStagedOrDBMedia(listingId) {
          try {
            ...
            // Signal which function is active (quick smoke-test for which copy runs)

            // Single-flight guard to avoid duplicate fetches for same listing
            try { window.__ffmMediaFetchInFlight = window.__ffmMediaFetchInFlight || new Map(); } catch (e) {}
            async function ffmGetMediaOnce(key, fn) {
              try {
                if (!window.__ffmMediaFetchInFlight) window.__ffmMediaFetchInFlight = new Map();
                if (window.__ffmMediaFetchInFlight.has(key)) return window.__ffmMediaFetchInFlight.get(key);
                const p = Promise.resolve().then(() => fn()).finally(() => { try { window.__ffmMediaFetchInFlight.delete(key); } catch (e) {} });
                window.__ffmMediaFetchInFlight.set(key, p);
                return p;
              } catch (e) { return fn(); }
            }

            const t0 = Date.now();
            const bgResp = await ffmGetMediaOnce(listingId, () => ffmRequestMediaViaPort({ listingId, userId: uid }));
            try { console.log('[MEDIA_PORT] PORT resp:', bgResp); } catch (e) {}

            const records = Array.isArray(bgResp?.result) ? bgResp.result : [];

            if (!Array.isArray(records) || records.length === 0) {
              console.warn('[Patch10] no media records', bgResp);
              return [];
            }
            ...
            try { console.log(`[MEDIA_PORT] got ${files.length} files for ${listingId} — first=${files[0] && files[0].name ? files[0].name : ''} type=${files[0] && files[0].type ? files[0].type : ''} elapsed=${elapsed}ms`); } catch (e) {}
            return files;
...
function ffmAttachMediaToFileInput(uploaderInput, files) {
  try {
    if (!uploaderInput) {
      console.warn("[AutoList Pro] ffmAttachMediaToFileInput: uploaderInput is null");
      return false;
    }
    if (!files || !Array.isArray(files) || files.length === 0) {
      console.warn("[AutoList Pro] ffmAttachMediaToFileInput: no files to attach");
      return false;
    }

    console.log("[AutoList Pro] ATTACH START — files:", files.length);

    const dt = new DataTransfer();

    for (const f of files) {
      console.log("[AutoList Pro] ATTACH — adding:", f.name, f.type, f.size);
      dt.items.add(f);
    }
    ...
    console.log("[AutoList Pro] ATTACH COMPLETE");
    return true;

  } catch (err) {
    console.error("[AutoList Pro] ffmAttachMediaToFileInput ERROR:", err);
    return false;
  }
}

        // 2) Title + Price must be present — treat as a hard phase (abort on failure)
        try {
          // Title
          console.log('[AutoList Pro] Waiting for Title input…');

          // Ensure the page switched into the edit form (Title/Price mode)
          const inEdit = await waitFor(() => {
            try { const txt = (document && document.body && document.body.innerText) ? document.body.innerText : ''; return txt.includes('Title') || txt.includes('Price'); } catch (e) { return false; }
          }, { timeoutMs: 15000 });
          if (!inEdit) {
            console.error('[AutoList Pro] ❌ Title/Price edit form never appeared — aborting publish');
            throw new Error('Edit form not detected');
          }

          const titleInput = await waitForMarketplaceInput(() => findMarketplaceInputByLabel('Title'));

          if (!titleInput) {
            console.error('[AutoList Pro] ❌ Title input not found — aborting publish');
            throw new Error('Title input not available');
          }

          await setAndConfirmInput({
            label: 'Title',
            element: titleInput,
            value: (listing.title && String(listing.title).trim()) || (listing.inventoryName && String(listing.inventoryName).trim()) || '',
            normalize: v => (v || '').toString().trim(),
            maxAttempts: 6,
            delayMs: 200
          });

          console.log('[AutoList Pro] ✅ Title confirmed', listing.title);

          // keep internal state consistent
          try { inventoryName = listing.title; } catch (e) {}

          // Price
          console.log('[AutoList Pro] Waiting for Price input…');

          const priceInput = await waitForMarketplaceInput(() => findMarketplaceInputByLabel('Price'));

          if (!priceInput) {
            console.error('[AutoList Pro] ❌ Price input not found — aborting publish');
            throw new Error('Price input not available');
          }
          ...
          await setAndConfirmInput({
            ...
            normalize: v => (v || '').toString().replace(/[^\d]/g, ''),
            maxAttempts: 8,
            delayMs: 200
          });

          console.log('[AutoList Pro] ✅ Price confirmed', listing.price);
        } catch (e) {
          console.error('[AutoList Pro] 🚫 Publish aborted due to critical input failure', e);
          return;
        }
        ...
        // Meetup preferences (Public meetup / Door pickup / Door dropoff) — only
        // touched if the listing has an opinion on at least one of them, so listings
        // saved before this feature existed don't get their FB defaults reset.
        try {
          await setMeetupPreferences(listing);
          console.debug('populate-fb: meetup preferences set');
        } catch (e) { console.warn('setMeetupPreferences error', e); }

        // Product tags / SKU — both live under FB's collapsed "More details"
        // disclosure, so expand it first if present (harmless if already open).
        try {
          await setProductTagsAndSku(listing);
          console.debug('populate-fb: product tags / sku set');
        } catch (e) { console.warn('setProductTagsAndSku error', e); }

        async function setProductTagsAndSku(listing) {
          if (!listing) return;
          const tagsRaw = (listing.productTags || '').toString().trim();
          const skuRaw = (listing.sku || '').toString().trim();
          if (!tagsRaw && !skuRaw) return;

          // Expand "More details" if it's collapsed.
          try {
            const moreBtn = Array.from(document.querySelectorAll('[role="button"], button')).find(
              (b) => (b.textContent || '').trim().startsWith('More details')
            );
            if (moreBtn) { moreBtn.click(); await sleep(300 + ffm_randInt(40, 120)); }
          } catch (e) { console.debug('setProductTagsAndSku: expand More details failed', e); }

          if (skuRaw) {
            try {
              const skuLabel = Array.from(document.querySelectorAll('label')).find(
                (l) => (l.textContent || '').trim().startsWith('SKU')
              );
              const skuInput = skuLabel ? skuLabel.querySelector('input,textarea') : null;
              if (skuInput) {
                skuInput.focus();
                skuInput.value = skuRaw;
                skuInput.dispatchEvent(new Event('input', { bubbles: true }));
                skuInput.dispatchEvent(new Event('change', { bubbles: true }));
                await sleep(150 + ffm_randInt(40, 120));
              } else { console.warn('[AutoList Pro content] SKU field not found'); }
            } catch (e) { console.warn('[AutoList Pro content] setProductTagsAndSku sku failed', e); }
          }

          if (tagsRaw) {
            try {
              const tagsLabel = Array.from(document.querySelectorAll('label')).find(
                (l) => (l.textContent || '').trim().startsWith('Product tags')
              );
              const tagsInput = tagsLabel ? tagsLabel.querySelector('input,textarea') : null;
              if (tagsInput) {
                ...
                for (const tag of tags) {
                  ...
                  await sleep(120 + ffm_randInt(30, 90));
                  tagsInput.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
                  tagsInput.dispatchEvent(new KeyboardEvent('keyup', { key: 'Enter', bubbles: true }));
                  await sleep(150 + ffm_randInt(40, 120));
                }
              } else { console.warn('[AutoList Pro content] Product tags field not found'); }
            } catch (e) { console.warn('[AutoList Pro content] setProductTagsAndSku tags failed', e); }
          }
        }

        // FB's "Meetup preferences" section: three role="checkbox" elements whose
        // textContent starts with the visible label (each also contains a sublabel/
        // link, so match by prefix rather than exact text).
        async function setMeetupPreferences(listing) {
          if (!listing) return;
          const hasOptIn = ['publicMeetup', 'doorPickup', 'doorDropoff'].some((k) => typeof listing[k] !== 'undefined');
          if (!hasOptIn) return;
          const wants = {
            'Public meetup': !!listing.publicMeetup,
            'Door pickup': !!listing.doorPickup,
            'Door dropoff': !!listing.doorDropoff
          };
          const checkboxes = Array.from(document.querySelectorAll('[role="checkbox"]'));
          for (const label of Object.keys(wants)) {
            try {
              const want = wants[label];
              const el = checkboxes.find((cb) => (cb.textContent || '').trim().startsWith(label));
              if (!el) { console.warn('[AutoList Pro content] meetup checkbox not found:', label); continue; }
              const checked = el.getAttribute('aria-checked') === 'true';
              if (checked === want) continue;
              try { el.scrollIntoView({ block: 'center' }); } catch (e) {}
              try { el.click(); } catch (e) { try { el.dispatchEvent(new MouseEvent('click', { bubbles: true })); } catch (e2) {} }
              await sleep(200 + ffm_randInt(40, 120));
            } catch (e) { console.warn('[AutoList Pro content] setMeetupPreferences item failed', label, e); }
          }
        }
  ...
  console.debug('populate-fb: setting additional attributes');
        ...
        async function setAttributeField(labelNames, value) {
          ...
          try {
            ...
            // try plain free-text input (e.g. Smart Home Compatibility has no dropdown, just a text box)
            const textInputEl = (control.tagName && control.tagName.toLowerCase() === 'input' && control.getAttribute('type') !== 'radio') ? control : (control.querySelector ? control.querySelector('input[type="text"], input:not([type])') : null);
            if (textInputEl && !selectEl && !radios.length) {
              try {
                textInputEl.focus();
                const setter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
                setter.call(textInputEl, value);
                textInputEl.dispatchEvent(new Event('input', { bubbles: true }));
                textInputEl.dispatchEvent(new Event('change', { bubbles: true }));
                console.debug('setAttributeField: set text input', labels, value);
                return;
              } catch (e) { console.debug('setAttributeField: failed to set text input', e); }
            }
                          ...
                          if (container) {
                            ...
                            try {
                              ...
                              if (numericMarkers && numericMarkers.length) {
                                // sort by proximity to labelEl if available
                                const byDist = (a, b) => {
                                  try {
                                    if (!labelEl) return 0;
        ...
        // NOTE: Facebook removed the Bed Size / Core Construction / Comfort Level / Mattress Type
        // detail fields from the posting form entirely (verified live, 2026-07-30) — they are no
        // longer attempted here since setAttributeField would just silently no-op looking for them.
        const attributeMap = {
          brand: ['Brand'],
          color: ['Color'],
          material: ['Material'],
          ...
          smartHomeCompatibility: ['Smart Home Compatibility']
        };
              ...
              if (autoChosen) {
                ...
                try {
                  if (typeof flag !== 'undefined' && flag !== null && ranked && ranked.length) {
                    // prefer a candidate whose underlying element text mentions 'hide' for hide=true
                    const lower = s => (s || '').toString().trim().toLowerCase();
                    let pickIdx = -1;
                    if (flag) {
                    ...
                    if (pickIdx < 0) {
                      // fallback: if flag=true choose first candidate, else choose second if exists
        ...
        await setAttributeField(attributeMap.color, getListingValue(['color','elecColor','elec_color']));
        await setAttributeField(attributeMap.material, getListingValue(['material','bikeMaterial','bike_material']));
        await setAttributeField(attributeMap.bikeType, getListingValue(['bikeType','bike_type']));
        await setAttributeField(attributeMap.wheelSize, getListingValue(['wheelSize','wheel_size']));
        await setAttributeField(attributeMap.smartHomeCompatibility, getListingValue(['smartHomeCompatibility','smart_home_compatibility']));

  // ✅ Universal fallback "Next" clicker (copied from the working 2927 logic)
  async function clickNextButton_AutoListPro(label = 'Next') {
    // Collect all visible "Next" buttons
    const nexts = Array.from(document.querySelectorAll(`div[aria-label="${label}"]`));
    if (!nexts.length) {
      console.warn('[AutoList Pro] No Next buttons found on page');
      return false;
    }
    ...
    console.log('[AutoList Pro] ✅ Clicked "Next" on attempt (HFF stage)', best);
    return true;
  }
  ...
  const loaded = await (async () => { try { return await clickNextButton_AutoListPro(); } catch (e) { return false; } })();
  if (!loaded) console.debug('populate-fb: delivery step did not load within timeout after clicking Next');
  try { console.log(`[AutoList Pro] Detected Marketplace layout variant: ${loaded ? 'A (Shipping)' : 'B (Local Only)'}`); } catch (e) {}
      ...
      try {
        ...
        try {
          ...
          async function clickMarketplaceNextButton() {
            ...
            if (!nextBtn) {
              console.warn('[AutoList Pro] ⚠️ No correct Next button found in composer');
              return false;
            }
            try { nextBtn.scrollIntoView({ behavior: 'smooth', block: 'center' }); } catch (e) {}
            await new Promise(r => setTimeout(r, 300));
            try { nextBtn.click(); console.log('[AutoList Pro] ✅ Clicked Marketplace Next button'); return true; } catch (e) { return false; }
          }
  ...
  // helper: persist a CSS-ish selector for the given element to ffm_selector_map
        ...
        async function ffmTryHardClick(el) {
          ...
          try {
            ...
            try { console.warn('[AutoList Pro] ffmTryHardClick failed:', err); } catch (e) {}
            return false;
          }
        }
      ...
      try {
        ...
        try {
          if (window.ffm_is_scheduled_publish === true) {
            try { console.log('[AutoList Pro content] HARD OVERRIDE: ffmAttemptPublishClick forcing allow=true (scheduler)'); } catch (e) {}
            try { window.ffm_allow_auto_publish = true; } catch (e) {}
          }
        } catch (e) {}
                ...
                while (attempt < max) {
                  ...
                  try {
                    ...
                    const body = (document && document.body && document.body.innerText) ? document.body.innerText.toLowerCase() : '';
                      ...
                      try { console.log(`[AutoList Pro] ✅ Listing published successfully at ${new Date().toLocaleTimeString()}`); } catch (e) {}
                      try {
        ...
        try {
          ...
          // ----- AutoList Pro HARD OVERRIDE: Always click Publish for scheduler -----
          try {
            if (window.ffm_is_scheduled_publish === true) {
              try {
                const pubBtn = document.querySelector('[aria-label="Publish"]');
                if (pubBtn) {
                  console.log('[AutoList Pro content] HARD OVERRIDE: Scheduler clicking Publish NOW');
    ...
    // After the delivery step appears (or we gave up waiting), attempt selection and advance
      ...
      async function ffmWaitFor(predicate, timeout = 3000, interval = 100) {
        try {
          ...
          try { console.warn('[AutoList Pro] ffmWaitFor timeout after', timeout); } catch (e) {}
          return false;
        } catch (e) { try { console.warn('[AutoList Pro] ffmWaitFor error', e); } catch (e2) {} return false; }
      }
      ...
      async function ffmTryHardClickQuick(el) {
        try {
          ...
          try { el.dispatchEvent && el.dispatchEvent(new MouseEvent('click', { bubbles: true })); } catch (e) {}
          return true;
        } catch (err) { try { console.warn('[AutoList Pro] ffmTryHardClickQuick failed', err); } catch (e) {} return false; }
      }
      ...
      try {
        ...
        if (cachedPrefs && typeof cachedPrefs === 'object') {
          ...
          if (!hasAnySelected) {
            console.log('[AutoList Pro] Skipping delivery checkbox phase — no delivery prefs selected.');
            ...
            console.log("[AutoList Pro] Waiting for delivery checkboxes to mount...");
      ...
      async function ffmApplyDeliveryOptions(options = {}) {
        ...
        for (const [label, want] of Object.entries({
          ...
          try {
            ...
            if (!el) {
              console.warn(`[AutoList Pro] ffmApplyDeliveryOptions: no checkbox found for ${label}`);
              continue;
            }

            const before = el.getAttribute && el.getAttribute('aria-checked') === 'true';
            console.log(`[AutoList Pro] ffmApplyDeliveryOptions: ${label} before=${before} want=${!!want}`);

            // Skip if already correct
            if (before === !!want) {
              console.log(`[AutoList Pro] ${label}: already in desired state`);
              continue;
            }
            ...
            if (after !== !!want) {
              ...
              try {
                ...
                console.log(`[AutoList Pro] ${label}: after fallback=${after2}`);
              } catch (kf) { console.debug('ffmApplyDeliveryOptions: keyboard fallback failed', kf); }
            } else {
              console.log(`[AutoList Pro] ${label}: after click=${after}`);
            }
      ...
      async function setFbDeliveryOptionsFromListing(listingOrId) {
        try {
          ...
          try { if (window && window.__ffm_skip_delivery_phase) { console.log('[AutoList Pro] setFbDeliveryOptionsFromListing: skipping — fast-path flagged'); return true; } } catch (e) {}
          ...
          try {
            ...
            if (!hasAnySelected) {
              console.log('[AutoList Pro] Skipping delivery checkbox phase — all unchecked.');
              return true;
            }
          } catch (e) {}
                    ...
                    chrome.storage.local.get(['ffm_saved_listings'], (data) => {
                      try {
                        ...
                        chrome.storage.local.set({ ffm_saved_listings: arr }, () => {
                          try { console.log('[AutoList Pro] 🟢 Listing marked active & saved to ffm_saved_listings for', ffmListing.inventoryName); } catch (e) {}
                          try {
                          ...
                          } catch (e) { console.debug('[AutoList Pro] failed scheduling refresh messages', e); }
                        });
                      } catch (e) { console.debug('[AutoList Pro] failed to persist active flag to ffm_saved_listings', e); }
                    });
                  } catch (e) { console.debug('[AutoList Pro] error reading/writing ffm_saved_listings storage', e); }
                } else {
                  try { console.warn('[AutoList Pro] ⚠️ No staged listing found to mark active'); } catch (e) {}
                }
              } catch (err) { console.error('[AutoList Pro] Failed to update active badge:', err); }
            } catch (e) { /* non-fatal */ }
    ...
    if ((message && message.scheduleTaskId) || (window && window.ffmCurrentPopulate && window.ffmCurrentPopulate.scheduleTaskId) || (window && window.ffm_staged_publish_request_id)) {
      try {
        ...
        if (window.ffmCurrentPopulate && window.ffmCurrentPopulate.scheduleTaskId) {
            console.log("[AutoList Pro] Scheduled publish: looking for final Publish button…");
            ...
            for (let i = 0; i < 15; i++) {
                ...
                if (publishBtn) {
                    console.log("[AutoList Pro] Scheduled publish: Publish button found → clicking.");
                    try { publishBtn.click(); } catch (e) { try { publishBtn.dispatchEvent(new MouseEvent('click',{bubbles:true})); } catch(_) {} }
                    break;
                }

                console.log("[AutoList Pro] Scheduled publish: Publish button not ready, retrying (" + i + "/15)…");
                await sleep(1000);
            }

            if (!publishBtn) {
                console.warn("[AutoList Pro] Scheduled publish: Publish button NEVER appeared.");
            }
        }
      } catch (e) { try { console.debug('[AutoList Pro] scheduled publish final-click patch error', e); } catch (er) {} }
    }
  } catch (e) {}
  try {
    const updateWhenOnSellingPage = () => {
      try {
        if (!location.href.includes('/marketplace/you/selling')) {
          try { console.log('[AutoList Pro content] Waiting for /you/selling before badge update...'); } catch (e) {}
          setTimeout(updateWhenOnSellingPage, 2000);
          return;
        }

        try { console.log('[AutoList Pro content] Detected /you/selling — performing badge merge'); } catch (e) {}
          ...
          try {
            ...
            try {
              if (publishId) {
                ...
                if (match) try { console.debug('[AutoList Pro content] Matched by publishRequestId', publishId); } catch (e) {}
              }
            } catch (e) {}
            ...
            if (!match) {
              try { console.warn('[AutoList Pro content] No direct match found — will retry after 3s'); } catch (e) {}
              ...
              chrome.storage.local.get('listings_version', (d) => {
                try {
                  ...
                  chrome.storage.local.set({ listings: saved, listings_version: version }, () => {
                    try { console.log('[AutoList Pro content] 🟢 Badge merge complete → bump version + broadcast'); } catch (e) {}
                ...
                } catch (e) { console.debug('[AutoList Pro content] failed to bump listings_version', e); }
              });
            } catch (e) { console.debug('[AutoList Pro content] listings_version flow failed', e); }
          } catch (err) { console.error('[AutoList Pro content] ❌ Badge merge failed', err); }
        });
      } catch (e) { console.debug('updateWhenOnSellingPage error', e); }
    };

    updateWhenOnSellingPage();
  } catch (err) { console.error('[AutoList Pro content] ❌ Failed to schedule badge merge', err); }
  ...
  if (window.ffm_allow_auto_publish) {
      ...
      console.error('[AutoList Pro] 🚫 Publish aborted due to critical failure', err);
      return;
    }
  }
...
(function() {
  try {
    ...
    function formatTemplate(tpl, ctx) {
      ...
      // Normalize tokens like ${Price}, {price}, {Price), {PRICE} -> {Price}
      tpl = tpl.replace(/(?:\$\{|\{)\s*([A-Za-z0-9_]+)\s*[\)\]\}]?/gi, (m, k) => {
        const key = (k || '').toString().trim();
        const low = key.toLowerCase();
        if (canonical[low]) {
          // preserve uppercase intent: {SIZE} -> {Size:UPPER}
              ...
              try {
                ...
                try {
                  chrome.runtime && chrome.runtime.onMessage && chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
                    try {
                        if (!message || message.action !== 'perform-generate-scrape') return;
                        try { console.debug && console.debug('[AutoList Pro content] perform-generate-scrape received from', sender && sender.tab ? sender.tab.id : sender); } catch (e) {}
                        (async () => {
                          try {
                            ...
                            try { console.debug && console.debug('[AutoList Pro content] perform-generate-scrape result', res); } catch (e) {}
                            try { sendResponse && sendResponse(res); } catch (e) { try { console.debug && console.debug('[AutoList Pro content] sendResponse failed', e); } catch (er) {} }
                          } catch (e) { try { console.debug && console.debug('[AutoList Pro content] perform-generate-scrape error', e); } catch (er) {} try { sendResponse && sendResponse(null); } catch (er) {} }
                        })();
