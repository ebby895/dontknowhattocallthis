...
(function () {
  ...
  // Retries a flaky FS Access API call before giving up. Covers two distinct
  // cases with the same backoff: a file just written moments earlier (e.g.
  // an antivirus/indexer scan) that throws transiently for a few hundred ms,
  // and — the much slower case — a folder restored/copied out of a Google
  // Drive (or OneDrive) sync that still contains cloud-only placeholder
  // files. Drive needs real time (easily several seconds per file) to
  // materialize those to disk, and throws NotFoundError until it does, so
  // this backs off exponentially up to ~16s total rather than giving up
